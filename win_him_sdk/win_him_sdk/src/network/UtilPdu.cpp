...
void CSimpleBuffer::Extend(uint32_t len)
{
	...
	unsigned char* new_buf = (unsigned char*)realloc(m_buffer, m_alloc_size);
    if(new_buf != NULL)
    {
    	m_buffer = new_buf;
    }
...
////// CByteStream //////
CByteStream::CByteStream(unsigned char* buf, uint32_t len)
{
	m_pBuf = buf;
	m_len = len;
	m_pSimpBuf = NULL;
...
int16_t CByteStream::ReadInt16(unsigned char *buf)
{
	int16_t data = (buf[0] << 8) | buf[1];
	return data;
}

uint16_t CByteStream::ReadUint16(unsigned char* buf)
{
	uint16_t data = (buf[0] << 8) | buf[1];
	return data;
}

int32_t CByteStream::ReadInt32(unsigned char *buf)
{
	int32_t data = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];
	return data;
}

uint32_t CByteStream::ReadUint32(unsigned char *buf)
{
	uint32_t data = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];
	return data;
}

void CByteStream::WriteInt16(unsigned char *buf, int16_t data)
{
	buf[0] = static_cast<unsigned char>(data >> 8);
	buf[1] = static_cast<unsigned char>(data & 0xFF);
}

void CByteStream::WriteUint16(unsigned char *buf, uint16_t data)
{
	buf[0] = static_cast<unsigned char>(data >> 8);
	buf[1] = static_cast<unsigned char>(data & 0xFF);
}

void CByteStream::WriteInt32(unsigned char *buf, int32_t data)
{
...
void CByteStream::WriteUint32(unsigned char *buf, uint32_t data)
{
	buf[0] = static_cast<unsigned char>(data >> 24);
	buf[1] = static_cast<unsigned char>((data >> 16) & 0xFF);
	buf[2] = static_cast<unsigned char>((data >> 8) & 0xFF);
...
void CByteStream::WriteData(unsigned char *data, uint32_t len)
{
	*this << len;
	_WriteByte(data, len);
}

unsigned char* CByteStream::ReadData(uint32_t &len)
{
	*this >> len;
	unsigned char* pData = (unsigned char*)GetBuf() + GetPos();
	Skip(len);
	return pData;
}
