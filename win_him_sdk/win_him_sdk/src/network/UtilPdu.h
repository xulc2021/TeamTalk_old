...
{
public:
	CByteStream(unsigned char* buf, uint32_t len);
	...
	static int16_t ReadInt16(unsigned char* buf);
	static uint16_t ReadUint16(unsigned char* buf);
	static int32_t ReadInt32(unsigned char* buf);
	static uint32_t ReadUint32(unsigned char* buf);
	static void WriteInt16(unsigned char* buf, int16_t data);
	static void WriteUint16(unsigned char* buf, uint16_t data);
	static void WriteInt32(unsigned char* buf, int32_t data);
	...
	void WriteData(unsigned char* data, uint32_t len);
	unsigned char* ReadData(uint32_t& len);
private:
